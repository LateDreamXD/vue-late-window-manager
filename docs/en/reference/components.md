...
Vue Late Window Manager provides two components: `LateWindow` and `LateWindowManager`. They will be registered as global components when [import plugin](../guide/quick-start#import).

If you did not want globally register these components, you can pass this option when import plugin<sup>`v0.3.0+`</sup>:

```ts [Disable Globally Register Components]
manager: {
  globallyRegisterComponts: false;
}
```
...
<template>
  <LateWindow :window="win">
    <template #default> write sth here... </template>
  </LateWindow>
</template>
...
```
